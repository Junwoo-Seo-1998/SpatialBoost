...
#define __INTERNAL_ASSERT_WITH_MSG(check, msg, ...) { if(!(check)) { Log::Critical("Assertion '{0}' failed at {1} line {2}\n*Detail Error Message:\n{3}", __STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).string(), __LINE__, msg); __DEBUGBREAK(); } }
#define __INTERNAL_ASSERT_NO_MSG(check) { if(!(check)) { Log::Critical("Assertion '{0}' failed at {1} line {2}", __STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).string(), __LINE__); __DEBUGBREAK(); } }

#define __INTERNAL_ENGINE_ASSERT_WITH_MSG(check, msg, ...) { if(!(check)) { EngineLog::Critical("Assertion '{0}' failed at {1} line {2}\n*Detail Error Message:\n{3}", __STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).string(), __LINE__, msg); __DEBUGBREAK(); } }
#define __INTERNAL_ENGINE_ASSERT_NO_MSG(check) { if(!(check)) { EngineLog::Critical("Assertion '{0}' failed at {1} line {2}", __STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).string(), __LINE__); __DEBUGBREAK(); } }
